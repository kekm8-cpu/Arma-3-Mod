...
// Routes are wiped while the entities that carry them still exist. They live
// on the objects, so most go with sync-back anyway, but a survivor carrying a
// stale route into the next battle would show up already committed to a plan
// the player made in the last one.
{
